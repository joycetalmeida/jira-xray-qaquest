...
function adfToText(node) {
  ...
  function getPrimarySprint(issue, sprintFieldId) {
    ...
    const sprint = fieldValue[fieldValue.length - 1];
    return {
      sprintId: sprint.id,
      sprintName: sprint.name,
      startDate: getDate(sprint.startDate),
      endDate: getDate(sprint.endDate)
    };
  }

  function upsertSprintScore(map, issue, sprintFieldId, points, type) {
    if (!sprintFieldId) return;
    const fieldValue = issue?.fields?.[sprintFieldId];
    if (!Array.isArray(fieldValue) || fieldValue.length === 0) return;

    // Use the FIRST sprint in the array (the sprint where the item was originally
    // introduced). This preserves completed-sprint history even when items are
    // carried over to subsequent sprints after sprint completion.
    const sprint = fieldValue[0];
    if (!sprint?.id) return;

    const key = String(sprint.id);
    if (!map.has(key)) {
      map.set(key, {
        sprintId: sprint.id,
...
export async function buildMyXrayGameReport({ projectKey, projectId, accountId }) {
  ...
  const execJql = `project = "${key}" AND issuetype = "${xrayExecType}" AND (creator = currentUser() OR reporter = currentUser() OR assignee = currentUser()) ORDER BY updated DESC`;
  const defectsJql = `project = "${key}" AND issuetype in ("${DEFECT_TYPES.join('", "')}") AND reporter = currentUser() ORDER BY updated DESC`;
  const plansJql = `project = "${key}" AND issuetype = "${xrayPlanType}" AND (creator = currentUser() OR reporter = currentUser() OR assignee = currentUser()) ORDER BY updated DESC`;
  const storiesJql = `project = "${key}" AND issuetype in ("${STORY_TYPES.join('", "')}") AND creator = currentUser() ORDER BY updated DESC`;
  const sprintItemsJql = `project = "${key}" AND issuetype in (${testTypeClause}, "${xrayExecType}") AND sprint in openSprints() AND (creator = currentUser() OR reporter = currentUser() OR assignee = currentUser()) ORDER BY updated DESC`;
  ...
  const [allTests, execs, defects, plans, stories, sprintItems, anySprintItems, sprintPerfFallbackItems] = await Promise.all([
  ...
  const storiesWithCoverage = stories.filter((s) => getLinksByType(s, [TEST_ISSUETYPE]).length > 0).length;
  ...
  const executions = execs.map((e) => {
    ...
    upsertSprintScore(sprintScoresMap, e, sprintFieldId, s.points, 'execution');
    return {
      key: e.key,
  ...
  for (const t of tests) {
    ...
    testPoints += s.points;
    fieldsFilled += s.filledFields;
    upsertSprintScore(sprintScoresMap, t, sprintFieldId, s.points, 'test');
  }
  ...
  if (sprintScoresMap.size === 0) {
    for (const item of sprintPerfFallbackItems) {
      ...
      upsertSprintScore(sprintScoresMap, item, sprintFieldId, s.points, isExecutionType ? 'execution' : 'test');
    }
